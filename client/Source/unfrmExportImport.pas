...
function Analyze: Integer;
  ...
  begin
    ...
    Result :=
       ...
       GetInt(DM.udCon, 'select count(*) as cnt from ListOtv4 where DoSend=1', [])+
       GetInt(DM.udCon, 'select count(*) as cnt from ListOtv9 where DoSend=1', [])+
       GetInt(DM.udCon, 'select count(*) as cnt from ListOtv11 where DoSend=1', [])>0;
    ...
    begin
      ...
      begin
        DS := GetDataSet(DM.udCon,
          ...
          ' lo.sAnswers1, lo.sAnswers2, lo.IDPredmet1, lo.IDPredmet2, '+
          ...
          begin
            ...
            begin
              ...
              arListOtv[DS.RecNo-1].sAnswers[PredmetIndex-1] :=
                DS.FieldByName('sAnswers'+IntToStr(PredmetIndex)).AsAnsiString;
            end;
      ...
      begin
        ...
        DS := GetDataSet(DM.udCon,
          ...
          ' lo.KPOIDSeason, lo.sVariant, lo.Variant, lo.DateTimeScan, lo.LiterClass, '+
          ' lo.IsEdit, lo.EditResult, lo.IsIden, lo.Blank, lo.Lang, lo.LOFileName, '+
          ' lo.sAnswers1, lo.sAnswers2, lo.sAnswers3, lo.IDPredmet1, lo.IDPredmet2, lo.IDPredmet3 '+
          'from ListOtv9 lo '+
          ' inner join Tests t on (t.IDTest=lo.IDTest) '+
          'where lo.DoSend=1 and t.Class9Status='+IntToStr(stResult)+' '+
          'order by lo.IDListOtv', []);
  ...
  function OnlineAnalyze: Boolean;
  var
    serv: GashOcen;
...
//    f.httprio.WSDLLocation := 'http://localhost:8080/gash_ocen/GashOcen?wsdl';
    f.httprio.URL := 'http://192.168.57.129:8080/gash_ocen/GashOcen';
    f.httprio.Service := 'GashOcenService';
    f.httprio.Port := 'GashOcenPort';

    serv := f.HTTPRIO as GashOcen;

    ansFile := nil;
    try
      try
        if not serv.isAlive then
          Exit;
      except
        Exit;
      end;
      ...
      begin
        ...
        ansFile.listotv[RecIndex].s_variant := arListOtv[RecIndex].sVariant;
        ...
        begin
          ...
          ansFile.listotv[RecIndex].answers[PredmetIndex] := arListOtv[RecIndex].sAnswers[PredmetIndex];
        end;
        ...
        ansFile.listotv[RecIndex].lo_filename := arListOtv[RecIndex].LOFileName;
      ...
      if AnalyzeResult<>nil then
      begin
        if (AnalyzeResult.result_code = 0) then
        begin
          ...
          try
            ...
            ExecuteSQL(DM.udCon, 'update ListOtv4 set DoSend=0', [], [], False);
            ExecuteSQL(DM.udCon, 'update ListOtv9 set DoSend=0', [], [], False);
            ExecuteSQL(DM.udCon, 'update ListOtv11 set DoSend=0', [], [], False);
            ...
            begin
              ...
              Application.MessageBox('При online оценке произошла ошибка.'#13+
                'Для оценки ответов будет сформирован файл.'#13+
                'Отправьте сформированный файл по почте.'#13+
                'После отправки в течение 2-3 минут вы получите файл с результатами оценки.',
  ...
  begin
    ...
    begin
      Application.MessageBox('Не удалось удалить ранее созданный файл.'#13+
...
begin
  ...
  try
    ...
    if not OnlineAnalyze then
    begin
      if CreateFileForAnalyze then
        Result := 2;
    end
    else
      Result := 1;
        ...
        try
          ...
          begin
            ...
            begin
              ...
              fdQry.ParamByName('Ocen1').AsIntegers[ParamIndex] := arOcen[Index].Ocen[0];
              fdQry.ParamByName('Ocen2').AsIntegers[ParamIndex] := arOcen[Index].Ocen[1];

              fdQry.ParamByName('Ball1').AsIntegers[ParamIndex] := arOcen[Index].Ball[0];
              fdQry.ParamByName('Ball2').AsIntegers[ParamIndex] := arOcen[Index].Ball[1];
          ...
          begin
            ...
            begin
              ...
              fdQry.ParamByName('Ocen1').AsIntegers[ParamIndex] := arOcen[Index].Ocen[0];
              fdQry.ParamByName('Ocen2').AsIntegers[ParamIndex] := arOcen[Index].Ocen[1];
              fdQry.ParamByName('Ocen3').AsIntegers[ParamIndex] := arOcen[Index].Ocen[2];

              fdQry.ParamByName('Ball1').AsIntegers[ParamIndex] := arOcen[Index].Ball[0];
              fdQry.ParamByName('Ball2').AsIntegers[ParamIndex] := arOcen[Index].Ball[1];
              fdQry.ParamByName('Ball3').AsIntegers[ParamIndex] := arOcen[Index].Ball[2];
          ...
          begin
            ...
            begin
              ...
              fdQry.ParamByName('Ocen2').AsIntegers[ParamIndex] := arOcen[Index].Ocen[1];
              fdQry.ParamByName('Ocen3').AsIntegers[ParamIndex] := arOcen[Index].Ocen[2];
              fdQry.ParamByName('Ocen4').AsIntegers[ParamIndex] := arOcen[Index].Ocen[3];

              fdQry.ParamByName('Ball1').AsIntegers[ParamIndex] := arOcen[Index].Ball[0];
              fdQry.ParamByName('Ball2').AsIntegers[ParamIndex] := arOcen[Index].Ball[1];
              fdQry.ParamByName('Ball3').AsIntegers[ParamIndex] := arOcen[Index].Ball[2];
              fdQry.ParamByName('Ball4').AsIntegers[ParamIndex] := arOcen[Index].Ball[3];
