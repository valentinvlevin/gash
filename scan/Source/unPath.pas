...
uses
  Messages, Windows, Classes, Forms, frxClass, unNSConst;
...
const
  ...
  procedure SetReportText(AReport: TfrxReport; const AMemoViewName, AText: string);

  function GetBatchFilePath: string;
...
begin
  ...
  SetCurrentDir(FAppPath+MainDir);
...
begin
  frxComponent:=AReport.FindObject(AMemoViewName);
  if frxComponent=nil
    then raise Exception.Create('Не найден объект '+AMemoViewName);
  if not (frxComponent is TfrxMemoView)
    then raise Exception.Create('Объект '+AMemoViewName+' не является TfrxMemoView');
  (frxComponent as TfrxMemoView).Text := AText;
  (frxComponent as TfrxMemoView).TagStr:='';
end;

function GetBatchFilePath: string;
begin
  {$IFDEF DEBUG}
  Result := 'z:\GASH\Projects\Scan\Batch\GASH.frs';
  {$ELSE}
  Result := GetAppPath + BatchDir + '\' + BatchFileName;
  {$ENDIF}
end;
...
begin
//  {$IFDEF DEBUG}
  Result := 'z:\GASH\scan\Data.stg';
//  {$ELSE}
//  Result := FAppPath + BinDir + '\' + DataFileName;
//  {$ENDIF}
end;
